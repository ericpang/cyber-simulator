...
class AppOrchestrator {
  ...
  async init() {
    ...
    try {
      const response = await fetch(`campaigns/ot/interactive_polls.json?v=${Date.now()}`);
      if (response.ok) {
        this.pollData = await response.json();
      } else {
        console.warn("campaigns/ot/interactive_polls.json response failed.");
      }
    } catch (err) {
      console.error("Failed to load campaigns/ot/interactive_polls.json config:", err);
    }

    // Load CTF challenges configuration
    try {
      const response = await fetch(`campaigns/ot/ctf_challenges.json?v=${Date.now()}`);
      if (response.ok) {
        this.ctfChallenges = await response.json();
      } else {
        console.warn("campaigns/ot/ctf_challenges.json response failed.");
      }
    } catch (err) {
      console.error("Failed to load campaigns/ot/ctf_challenges.json config:", err);
    }
  ...
  bindControls() {
    ...
    this.scenarios.forEach(sc => {
      ...
      if (btn) {
        const shortNames = ["CAMP-01: POWER GRID", "CAMP-02: TRANSIT SYSTEM", "CAMP-03: HOSPITAL ER"];
